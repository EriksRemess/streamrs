fn load_svg_data(
    ...
    let tree = usvg::Tree::from_data(svg_data, &options)
        .map_err(|err| format!("Failed to parse SVG '{label}': {err}"))?;

    let mut pixmap = tiny_skia::Pixmap::new(target_w, target_h)
        .ok_or_else(|| format!("Failed to allocate render target for '{label}'"))?;
    ...
    let transform = tiny_skia::Transform::from_scale(scale, scale).post_translate(x_offset, y_offset);

    resvg::render(&tree, transform, &mut pixmap.as_mut());

    RgbaImage::from_raw(target_w, target_h, pixmap.take())
        .ok_or_else(|| format!("Failed to create image for '{label}'"))
}
...
fn render_blank_base(
    ...
    let rendered = load_svg_data(label, svg_data, resources_dir, render_w, render_h)?;
...
fn detect_key_slots(base: &RgbaImage) -> Result<Vec<KeySlot>, String> {
...
fn scale_slots(template_slots: &[KeySlot], width: u32, height: u32) -> Vec<KeySlot> {
...
fn fallback_slots(width: u32, height: u32) -> Vec<KeySlot> {
...
fn deck_background_temp_path() -> PathBuf {
