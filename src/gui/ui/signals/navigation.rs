...
pub(crate) fn wire_navigation_signals(ctx: &UiCtx) {
    let state = &ctx.state;
    ...
    for (index, button) in key_buttons.iter().enumerate() {
        ...
        button.connect_clicked(move |_| {
            ...
            let nav_slot = {
                let mut state = state_for_click.borrow_mut();
                normalize_config(&mut state.config);
                navigation_slot_for_slot(&state.config, page, total_pages, index)
            };
...

        {
            ...
            drop_target.connect_drop(move |_, value, x, _| {
                ...
                announce_status(&widgets_for_drop, &operation_message);
                true
            });
